...
def mostrar_peliculas (lista_peliculas: dict) -> None:
    ...
    matriz = [
            ["Titulo", "Genero", "Año de lanzamiento", "Duración", "ATP", "Plataforma"] # crea cabecera
        ]
...
lista_peliculas[i]["titulo"],lista_peliculas[i]["genero"],str(lista_peliculas[i]["año_lanzamiento"]),str(lista_peliculas[i]["duracion"]),"Si" if lista_peliculas[i]["atp"] else "No",lista_peliculas[i]["plataforma"]]
            matriz.append(fila) # añade filas

    print(caracteres_externa)
    for i in range (len(matriz)):
            print(f"| {matriz[i][0]:<50} | {matriz[i][1]:<25} | {matriz[i][2]:<18} | {matriz[i][3]:<8} | {matriz[i][4]:<3} | {matriz[i][5]:<30} |") # formato
            if i == 0:
...
def pelicula_titulo (lista_peliculas: list) -> None:
    ...
    encontrado = False
    for i in range(len(lista_peliculas)):
        if titulo_buscado.lower() == lista_peliculas[i]["titulo"].lower():
            mostrar_peliculas_por_atributo(lista_peliculas, "titulo", lista_peliculas[i]["titulo"])
            encontrado = True
            break
    if not encontrado:
        print("No se encontró la película especificada.")


def mostrar_peliculas_por_atributo(lista_peliculas: list, atributo: str, valor_busqueda) -> None:
    ...
    Args:
        ...
        valor_busqueda: El valor específico que se busca en el atributo.
    Returns:
        None
    """
    encontrada = False  
    ...
    for i in range(len(lista_peliculas)):
        ...
        val = pelicula.get(atributo)

        if isinstance(val, str) and isinstance(valor_busqueda, str):
            match = val.strip().lower() == valor_busqueda.strip().lower()
        else:
            match = val == valor_busqueda
...
def mostrar_por_genero (lista_peliculas) -> None:
    ...
    path = os.path.join(dir_path, "peliculas_generos.json")
...
def mostrar_peliculas_por_plataforma(lista_peliculas: list, atributo: str, valor_busqueda: str) -> None:
    ...
    for i in range(len(lista_peliculas)):
        ...
        if val and isinstance(val, str) and valor_busqueda.strip().lower() in val.strip().lower():
            peliculas_encontradas.append(pelicula)
            encontrada = True
