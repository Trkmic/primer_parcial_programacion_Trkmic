...
csv_path = os.path.join(dir_path, "peliculass.csv")

lista_peliculas = cargar_desde_csv(csv_path)
...
def dar_alta () -> dict:
    ...
    año_lanzamiento = validar_numero("Ingrese el año de lanzamiento: ", lambda numero: 1888 <= numero <= año_actual)
...
def modificar_pelicula (lista_peliculas: list[dict]) -> str:
    """_summary_
    ...
    if not lista_peliculas:
        return "Error. No hay películas cargadas."
    ...
    for i in range(len(lista_peliculas)):
        if verificar_titulo.lower() == lista_peliculas[i]["titulo"].lower():
            bandera_existe = True
            while True:
                    ...
                    match opcion:
                        ...
                        case "c":
                            dato = validar_numero("Ingrese el nuevo año de lanzamiento de la pelicula: ", lambda x: 1888 <= x <= año_actual)
...
def submenu_peliculas (lista_peliculas: list[dict]) -> None:
    """ _summary_
        Es un submenu el cual podrias mostrar diferentes datos sobre películas, como todas las películas,películas de un género específico, películas de un año específico, películas aptas para todo público, películas no aptas para todo público o salir del menú.
    Args:
        lista_peliculas (list[dict]): Una lista que contiene información sobre las películas.
    Return:
        None
    """
    ...
    while True:
            ...
            match opcion:
                ...
                case "c":
                    año_actual = datetime.datetime.now().year
                    anio_lanzamiento_buscado = validar_numero("Ingrese el año de lanzamiento que busca: ", lambda x: 1888 <= x <= año_actual)
...
def ordenar_peliculas (lista_peliculas:list) -> None:
    """_summary_
...
def submenu_promedios (lista_peliculas: list[dict]) -> None:
    """_summary_
        Muestra el promedio del atributo seleccionado por el usuario.
    Args:
        lista_peliculas (list[dict]): Una lista que tiene información sobre las películas.
    Return:
        None
    """
...
def submenu_porcentajes (lista_peliculas: list[dict]) -> None:
    """_summary_
