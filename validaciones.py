# validaciones.py (No custom imports needed here to avoid circular dependencies)
...
def dar_alta_titulo(lista_peliculas:list,mensaje:str, criterio:bool) -> str:
    ...
    while True:
        ...
        repetido = False
        for i in range(len(lista_peliculas)):
            if lista_peliculas[i]["titulo"].strip().lower() == cadena.lower():
                repetido = True
                break
...
def validar_titulo(mensaje, criterio) -> str:
    ...
    while not criterio(cadena):
        print("Ingrese una opción válida por favor!")
        cadena = input("Ingrese la opción nuevamente: ").strip().capitalize()
...
def validar_numero(mensaje, criterio) -> int:
    ...
    while not numero.isdigit() or not criterio(int(numero)):
        ...
        numero = input("Ingrese el valor nuevamente: ").strip()
    return int(numero)
...
def validar_genero() -> str:
    ...
    lista_generos = ["Acción", "Aventura","Animación", "Biográfico", "Comedia", "Comedia romántica", "Comedia dramática", "Crimen", 
                    "Documental", "Drama", "Fantasía", "Histórico", "Infantil", "Musical", "Misterio", "Policíaco", "Romance", "Ciencia ficción", "Suspenso", "Terror", "Western", 
                    "Bélico", "Deportivo", "Noir", "Experimental", "Familiar", "Superhéroes", "Espionaje", "Artes marciales", "Fantástico", "Catástrofe", "Melodrama", "Erótico", 
                    "Cine independiente", "Zombies", "Vampiros", "Cyberpunk", "Steampunk", "Distopía", "Utopía", "Road Movie", "Docuficción", "Mockumentary", "Gótico", "Slasher", 
                    "Adolescente", "Culto", "Maravilloso"]
    ...
    while True:
        ...
        for i in range(len(lista_generos)):
            if genero.lower() == lista_generos[i].lower():
                genero = lista_generos[i]
                genero_encontrado = True
                break
        
        if genero_encontrado == True:
            return genero
        else:
            print("Ingrese una opción válida por favor!")
...
def validar_si_no(mensaje) -> bool:
    ...
    atp = input(mensaje).strip().capitalize()
...
def validar_plataformas(mensaje:str) -> list[str]:
    ...
    while True:
        ...
        ingresar_otra_plataforma = input("Esa fue su ultima plataforma o quiere agregar otra? (Responda con Si o No)?: ").strip().capitalize()
        while (ingresar_otra_plataforma != "Si" and ingresar_otra_plataforma != "No"):
            ingresar_otra_plataforma = input("Ingrese una opcion valida: ").strip().capitalize()
